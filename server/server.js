...
app.put(
  "/api/quotations/:quotationId/upload-client-proof",
  upload.single("file"),
  async (req, res) => {
    try {
      if (!req.file) {
      ...
      const updated = await db.collection("quotations").findOneAndUpdate(
        { quotationId },
        {
          $set: {
            ...
            signedFileName: req.file.originalname, // for a reliable download filename
...
// ==========================
// STANDARDS (Calibration Standards / Assets — AddAssetModal)
// ==========================
// Paste this whole block into server.js, ABOVE the
// `const PORT = process.env.PORT || 3000;` line at the bottom.
//
// One "standards" collection is the single source of truth. Every saved
// standard shows up on ALL of: Asset Monitoring, Standard For Calibration,
// Standard For Certification, and Standard For Update — these are just
// different views onto the same full list, not status-filtered subsets.
// (The dedicated /for-calibration, /for-certification, /for-update routes
// below are kept as separate endpoints purely so each page's frontend
// code stays independent — if you ever do want to split them by status
// later, that's the only place you'd need to change.)

// GET next Standard ID preview (mirrors /api/jobreceipts/next-id)
app.get("/api/standards/next-id", async (req, res) => {
  try {
    const counter = await db
      .collection("counters")
      .findOne({ _id: "standardID" });
    const nextSeq = (counter?.seq || 0) + 1;
    const nextStandardId = `STD-${String(nextSeq).padStart(4, "0")}`;
...
app.get("/api/standards/for-calibration", async (req, res) => {
  try {
    const standards = await db
      .collection("standards")
      .find()
...
app.get("/api/standards/for-certification", async (req, res) => {
  try {
    const standards = await db
      .collection("standards")
      .find()
...
app.get("/api/standards/for-update", async (req, res) => {
  try {
    const standards = await db
      .collection("standards")
      .find()
...
// "Modification History" buttons in AddAssetModal. Kept as one combined
// log (action: "Created" | "Updated" | "Flagged For Calibration"); the
// frontend can filter by `action` client-side if it wants two distinct
// views out of the one list.
// NOTE: registered before the generic "/:standardId" route below so
// Express doesn't try to match "history" as part of a standardId.
app.get("/api/standards/:standardId/history", async (req, res) => {
  try {
...
// GET a single standard by standardId (used to open AddAssetModal
// pre-filled with existing data)
app.get("/api/standards/:standardId", async (req, res) => {
  try {
...
// POST create a new standard — Standard ID is generated atomically here
// with the same counter pattern as customerID / jobReceiptID, so the
// client-side value (if any) is never trusted as the source of truth.
app.post("/api/standards", async (req, res) => {
  try {
    ...
    if (!standardId || !standardId.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Standard ID is required." });
    }

    const existing = await db
      .collection("standards")
      .findOne({ standardId: standardId.trim() });
...
// PUT update an existing standard by standardId — used by AddAssetModal's
// "Update" button (form submit). Diffs against the previous doc and logs
// a modification history entry. If the save changes calibStatus, the
// record simply starts showing up under the matching list page on next
// fetch — no extra bookkeeping required.
app.put("/api/standards/:standardId", async (req, res) => {
  try {
    ...
    await db
      .collection("standards")
      .updateOne({ standardId }, { $set: updateData });
...
// PUT quick-action: flag a standard as due for calibration — mirrors
// AddAssetModal's "For Calibration" button. Sets calibStatus so the
// record moves into the "Standard For Calibration" list.
app.put("/api/standards/:standardId/for-calibration", async (req, res) => {
  try {
    const standardId = decodeURIComponent(req.params.standardId);
...
// DELETE a standard
app.delete("/api/standards/:standardId", async (req, res) => {
  try {
    await db
      .collection("standards")
      .deleteOne({ standardId: decodeURIComponent(req.params.standardId) });
