 ...
 *
 * OIC (oicBy) is always the currently logged-in user (from sessionStorage),
 * not a manually-picked value — it reflects whoever is actually processing
 * the job right now. It's re-stamped every time this modal is opened, and
 * whatever value is showing when "Update" is clicked is what gets saved.
 *
...
// // TODO: replace with your real list of OICs, or fetch from an API
// // the same way CONTACT_CERT_OPTIONS could be.
// const OIC_OPTIONS = ["CPGP", "JPR", "MCJ"];
...
//  * "ON-GOING CALIBRATION DETAILS") without duplicating the component.
//  *
//  * OIC (oicBy) is a manually-picked selection — whoever is assigned to
//  * process the job — not automatically tied to the logged-in user.
//  *
//  * NOTE: sig, dateCal, dateDue, calibrationProcedure, accreditationLogo,
//  * calibrationStandards, and photoUrl are NOT part of the record shape
//  * produced by IncomingCalib.jsx today. They're initialized here from
...
//     frequency: jobForm.frequency || "1 Year",
//     priority: jobForm.priority || "Normal",
//     // OIC is now a manually-picked selection (who is actually assigned to
//     // process this job), not auto-filled from the logged-in user.
//     oicBy: jobForm.oicBy || "",
...
//   // OIC options: same safeguard — keep an existing value visible even if
//   // it's not in the hardcoded list above.
//   const oicOptions = OIC_OPTIONS.includes(form.oicBy)
//     ? OIC_OPTIONS
//     : [form.oicBy, ...OIC_OPTIONS].filter(Boolean);
...
//               <div className="icd-inline-field">
//                 <label>OIC</label>
//                 <select name="oicBy" value={form.oicBy} onChange={handleChange}>
//                   <option value="">-- Select --</option>
//                   {oicOptions.map((opt) => (
//                     <option key={opt} value={opt}>
//                       {opt}
